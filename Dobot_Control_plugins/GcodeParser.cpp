...
//    segment.line = m_Gcode_Q->size();
    ...
    segment.data_xyz = this->m_currentPoint + m_offsetPoint;
...
//    segment.speed_spindle = this->m_lastSpindleSpeed;
//    segment.speed_step = this->m_lastSpeed;
    segment.data_xyz = nextPoint+m_offsetPoint;
    m_Gcode_Q->enqueue(segment);
...
void GcodeParser::setOffsetPoint(const QVector3D point)
{
    m_offsetPoint = point;
}
