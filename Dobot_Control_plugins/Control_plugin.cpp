    ...
    QObject(parent),EtherCAT_UserApp()
{
    ...
    this->set_AppName("Dobot Control");
...
//        GCodeItem item;
        gp_t->clearQueue();
        //NOTE:dobot这里加上了初始值,G代码的第一行不需要再次写初始位置了
        QVector3D curPoint(0,
                m_motorApp_callback->m_PositionInit->arm[Dobot_Motion::AXIS_Y],
                m_motorApp_callback->m_PositionInit->arm[Dobot_Motion::AXIS_Z]);
        gp_t->setCurrentPoint(curPoint);
        gp_t->setOffsetPoint(QVector3D(m_motorApp_callback->m_PositionInit->arm[Dobot_Motion::AXIS_X],0,0));
