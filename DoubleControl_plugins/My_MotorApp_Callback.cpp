...
//    *(input_ptr+motor3_step) = 1;
    ...
    m_DobotMotion.m_Stepper_control->step_count = *(m_DobotMotion.get_Output_MotorCountPtr());
    if(m_sys_reset){//如果用户进行了复位
       ...
       Motor_Reset();
       *(m_DobotMotion.get_Iutput_MotorStepPtr()+AXIS_TOOL1) = 0;
    }

    m_DobotMotion1.m_Stepper_control->step_count = *(m_DobotMotion1.get_Output_MotorCountPtr());
    if(m_sys_reset){//如果用户进行了复位
       ...
       Motor_Reset();
       *(m_DobotMotion1.get_Iutput_MotorStepPtr()+AXIS_TOOL1) = 0;
    }
...
//    }
    if(m_DobotMotion.m_RenewST_init){//给一个循环运动的激励
        if((*(m_DobotMotion.get_OutputPtr()+motor_MSG)==2) || m_DobotMotion.m_McodeFlag){
    ...
    if(m_DobotMotion1.m_RenewST_init){//给一个循环运动的激励
        if((*(m_DobotMotion1.get_OutputPtr()+motor_MSG)==2) || m_DobotMotion1.m_McodeFlag){
...
//    input_MotorStep_ptr = (uint32_t*)(m_Master_addressBase+0x06);
    ...
    m_DobotMotion.set_Output_MotorCountPtr(output_MotorCount_ptr);
...
//        qDebug() << "slaveCont=" << Master_getSlaveCount();
        ...
        input_MotorStep_ptr = (uint32_t*)(m_Master_addressBase+m_Master_addressList[1].outputs_offset+0x08);
        output_MotorCount_ptr = (uint32_t*)(m_Master_addressBase+m_Master_addressList[1].inputs_offset);
...
/// \return
///
int My_MotorApp_Callback::Planner_BufferLine(float * target,Gcode_segment &userData,int robot_index){
...
void My_MotorApp_Callback::Arm_motion_reset(){
    m_DobotMotion.Arm_motion_reset(m_isMode_calibrate);
    m_DobotMotion1.Arm_motion_reset(m_isMode_calibrate);
