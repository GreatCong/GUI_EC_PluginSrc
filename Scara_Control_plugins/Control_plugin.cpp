    ...
    QObject(parent),EtherCAT_UserApp()
{
    ...
    this->set_AppName("Scara Control");
...
bool Control_plugin::Init_Cores()
{
    ...
    user_form_controlTab->get_LineEditGcode(Form_ControlTab::Jog_PosX_e)->setText(QString("%1").arg(m_motorApp_callback->m_PositionInit->arm[Scara_Motion::AXIS_X]));
    user_form_controlTab->get_LineEditGcode(Form_ControlTab::Jog_PosX_e)->setPlaceholderText(tr("X轴"));
    user_form_controlTab->get_LineEditGcode(Form_ControlTab::Jog_PosY_e)->setText(QString("%1").arg(m_motorApp_callback->m_PositionInit->arm[Scara_Motion::AXIS_Y]));
    user_form_controlTab->get_LineEditGcode(Form_ControlTab::Jog_PosY_e)->setPlaceholderText(tr("Y轴"));
    user_form_controlTab->get_LineEditGcode(Form_ControlTab::Jog_PosZ_e)->setText(QString("%1").arg(m_motorApp_callback->m_PositionInit->arm[Scara_Motion::AXIS_Z]));
...
//        m_motorApp_callback->start();
        break;
     case Qt::Key_R:
            m_motorApp_callback->m_ARM_Motion_test->arm[My_MotorApp_Callback::AXIS_X] = m_motorApp_callback->m_PositionInit->arm[Scara_Motion::AXIS_X];
            m_motorApp_callback->m_ARM_Motion_test->arm[My_MotorApp_Callback::AXIS_Y] = m_motorApp_callback->m_PositionInit->arm[Scara_Motion::AXIS_Y];
            m_motorApp_callback->m_ARM_Motion_test->arm[My_MotorApp_Callback::AXIS_Z] = m_motorApp_callback->m_PositionInit->arm[Scara_Motion::AXIS_Z];
//            m_motorApp_callback->start();
            segment.speed_step = user_form_controlTab->get_LineEditGcode(Form_ControlTab::Jog_Speed_e)->text().toFloat();//设置步进电机的转速
            m_motorApp_callback->Planner_BufferLine(m_motorApp_callback->m_ARM_Motion_test->arm,segment,0);
            m_motorApp_callback->m_sys_reset = false;
